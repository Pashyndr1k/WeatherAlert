...
<p><b>Colour grading</b> (Settings → Alerts &amp; units → Widget colour): off, colour the main number, or colour the widget background. The tint follows how close the value is to its limit using the app's own scale, green far below, amber approaching, red at or beyond; widgets without a limit stay neutral and alarm states always take precedence. Hovering a widget outlines it.</p>
...
<p><b>Layout</b>: Settings → Map → Widget layout switches between the <b>right panel</b> (default: two columns of widgets beside the map) and the <b>bottom strip</b> (map-first).</p>` },
...
<p><b>0.4.3</b> — widget hover outline; value-graded widget colouring (off / number / background).</p>
<p><b>0.4.2</b> — provider menu in the top bar now opens below the bar (it was clipped).</p>
<p><b>0.4.1</b> — right-panel widget layout is now the default.</p>
...
<p><b>Градація кольору</b> (Налаштування → Сповіщення й одиниці → Колір віджетів): вимкнено, забарвлювати головне число або фон віджета. Відтінок відповідає близькості значення до межі за власною шкалою застосунку: зелений — далеко, жовтий — наближення, червоний — на межі або за нею; віджети без межі лишаються нейтральними, стани тривог мають пріоритет. Наведення на віджет підсвічує його контур.</p>
...
<p><b>Розкладка</b>: Налаштування → Карта → Розкладка віджетів перемикає між <b>правою панеллю</b> (типово: два стовпці віджетів поруч із картою) і <b>нижньою смугою</b> (карта на весь екран).</p>` },
...
<p><b>0.4.3</b> — підсвічування контуру віджета при наведенні; забарвлення віджетів за значенням (вимкнено / число / фон).</p>
<p><b>0.4.2</b> — меню джерел у верхній панелі тепер відкривається під панеллю (раніше обрізалося).</p>
<p><b>0.4.1</b> — права панель віджетів тепер типова розкладка.</p>
...
<p><b>0.1.0</b> — перший випуск: карта Чорного моря GSHHG, джерела Open-Meteo / Stormglass, порогові тривоги з упередженням 60–90 хв, редагування меж на віджетах, лаунчер і збірки релізів на GitHub.</p>` }
  ...
  WA.GUIDE = { version: '0.4.3', en: EN, uk: UK };
})();
