...
## Interface (v0.2 — "OUTPOST")

Map-first tactical layout from the Claude Design handoff (option 1A): 56 px top bar with UTC clock and
SND / RFR / ALM / SET buttons, tracking-point rail on the map, selected-point metric strip along the
bottom, full-screen **Active alarms** view (table + 90-minute timeline) and full-screen **Settings**
with a left tab rail. Rajdhani + IBM Plex Mono are bundled in `assets/fonts` for offline use.
...
- **Maritime derived indicators**: Beaufort force, Douglas sea state, cloud oktas, air conditions (fog / mist / haze / rain / snow / thunderstorm), 3-hour pressure tendency, cross-sea detection, fog risk (dew-point spread), gust factor, plus two alarm-capable indices:
  - **Icing index** — Overland (1990) predictor `PPR = V·(Tf − Ta) / (1 + 0.3·(Tw − Tf))` from wind, air and water temperature, converted to an icing rate in cm/h with light / moderate / heavy / extreme classes. Computed everywhere; flagged as the standard ≥ 55°N zone or "advisory" south of it. Default alarm ≥ 0.7 cm/h (moderate).
  - **Advection-fog probability** — dew point against sea-surface temperature (warm moist air over colder water), shaped by humidity and the 2–8 m/s wind band, as a 0–100 % score. Default alarm ≥ 60 %.
